...
The `DelegateHolder` class contains a delegate member variable `Greeting` that is set in the [constructor](classes.md#constructors-and-destruc) to the member function `Introduce()`.
...
The `MyMathLib` class contains just a single [static](attributes.md#static) [function](functions.md) that returns the product of exponentiation. This function will be used to demonstrate how to use a delegate in the next and final class.
...
IMPORTANT: Delegates that point at member functions can create memory leaks if they make cycles. See [memory_management](memory_management.md) for more.
