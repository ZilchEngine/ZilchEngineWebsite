...
NOTE: The types `HashMap` and `KeyValue` not currently appear in the [nada_base_types](../../../code_reference/nada_base_types.md) list and does not currently show up in code completion suggestions (T552) during initial declaration.
...
```
 ...
 ### HashMap.GetOrError
`GetOrError` has the same functionality as [Get](hashmap/.md#hashmap-get), the only difference being that it is not called by the `[]` operator.

 # Adding to a HashMap
There are multiple ways to add `KeyValue` pairs to HashMaps. [Add](hashmap/.md#hashmap-add), [Set](hashmap/.md#hashmap-set), and the `[]` operator can all be used to add `KeyValue` pairs to the HashMap.
...
```
 ...
 ### HashMap.SetOrError
`SetOrError` follows a similiar behavior to [GetOrError](hashmap/.md#hashmap-getorerror). If the key passed to `SetOrError` already exists in the HashMap then an exception will be thrown.
...
- [Arrays](arrays.md)
 ## Reference
- [Arrays](../../../code_reference/nada_base_types/array_t.md)
